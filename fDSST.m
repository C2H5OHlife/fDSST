...
nScales = params.number_of_scales; %fDSST的scale从31个变为17个
...
for frame = 1:num_frames,
    ...
    % just want to initialize the tracker there 第一帧不估计
    if frame > 1
        ...
        if nScales > 0
            ...
            %[xs_pca] 744x17 [xs] 17x17 [xs_npca] 空
    ...
    if nScales > 0
        ...
        [xs_pca, xs_npca] = get_scale_subwindow(im, pos, base_target_sz, currentScaleFactor*scaleSizeFactors, scale_model_sz);
        % 上面一句提取了17个不同尺度的patch再统一resize到scale_model_sz大小,然后再分别提取这些patch的hog特征，将hog特征拉成一维的
        % [xs_pca] 744x17 一列是一个尺度样本的特征向量 [xs] 17x17 [xs_npca] 空
